...
router.get('/get-items/:item_id', async (req, res) => {
  try {
...
router.post('/create-items', async (req, res) => {
  try {
    ...
    res.status(201).json({item_id, item_name, item_type, item_description, item_stock, item_image, item_price});
  } catch(err) {
    res.status(500).json({'message': 'failed to create item'});
  }
})

router.patch('/update-items/:item_id', async (req, res) => {
  try {
    const {item_id} = req.params;
...
router.delete('/delete-items/:item_id', async (req, res) => {
  try {
    const {item_id} = req.params;
    const [result] = await getPool().query(
      `DELETE FROM items 
